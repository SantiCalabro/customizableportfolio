...
const Dropdown = () => {
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      {isOpen.dev && (
        <div
          className="absolute right-0 py-10 w-full origin-top-right bg-dark shadow-lg ring-1 ring-black ring-opacity-5 transition-all duration-300 ease-in-out max-h-[500px] opacity-100 overflow-hidden"
          style={{ maxHeight: isOpen.dev ? "500px" : "0", opacity: isOpen.dev ? 1 : 0 }}
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 sm:px-4 md:grid-cols-3 md:px-8 lg:grid-cols-4 lg:px-10 xl:grid-cols-5 2xl:grid-cols-7 2xl:px-40 gap-4 place-items-center">
            {Array.from({ length: 10 }).map((_, index) => (
              <Chip key={index} icon={JsIcon} alt="JavaScript" tech="JavaScript" />
            ))}
          </div>
        </div>
      )}
      ...
      {isOpen.design && (
        <div
          className="absolute right-0 py-10 w-full origin-top-right bg-dark shadow-lg ring-1 ring-black ring-opacity-5 transition-all duration-300 ease-in-out max-h-[500px] opacity-100 overflow-hidden"
          style={{ maxHeight: isOpen.design ? "500px" : "0", opacity: isOpen.design ? 1 : 0 }}
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 sm:px-4 md:grid-cols-3 md:px-8 lg:grid-cols-4 lg:px-10 xl:grid-cols-5 2xl:grid-cols-7 2xl:px-40 gap-4 place-items-center">
            {Array.from({ length: 10 }).map((_, index) => (
              <Chip key={index} icon={JsIcon} alt="JavaScript" tech="JavaScript" />
