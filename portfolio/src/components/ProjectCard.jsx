...
export default function ProjectCard({img, alt, title, description, type, handleUpdate, filterState, el }) {
  ...
  return (
      ...
      <div className="border-primary-gray flex relative rounded-lg overflow-hidden">
        <div className="commands absolute right-0 top-0 flex bg-mandarine w-fit gap-3 p-2 rounded-tr-md rounded-bl-md">
                 <img onClick={()=>handleUpdate(el.id)} src={editPen} alt="edit" className="cursor-pointer pr-2 border-r border-black"/>
                 <img onClick={()=>filterState(el.id)} src={blackCross} alt="delete" className="cursor-pointer"/>
            </div>
        ...
        <div className="w-2/3 flex flex-col p-5">
          ...
          <h4 className=" mt-4 white font-bold text-2xl w-full">{title}</h4>
